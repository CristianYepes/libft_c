...
let functionIndex: FunctionInfo[] | null = null;

function getFunctionIndex(): FunctionInfo[] {
  if (!functionIndex) {
...
server.tool(
  "function_lookup",
  "Semantic search across all libft + GNL functions. Returns prototype, category, allocation info, and dependencies.",
  {
...
server.tool(
  "dependency_graph",
  "Show the call graph for a function — what it calls (callees) and what calls it (callers) within the library.",
  {
  ...
  async ({ function_name, direction = "both", depth = 3 }) => {
    ...
    const lines: string[] = [`# Dependency Graph: ${name}\n`];

    if (direction === "callees" || direction === "both") {
      lines.push("## Callees (what this function calls)\n");
      const visited = new Set<string>();
      function traceCallees(fn: FunctionInfo, level: number): string[] {
        if (level > depth || visited.has(fn.name)) return [];
        visited.add(fn.name);
        const result: string[] = [];
...
server.tool(
  "test_against_libc",
  "Compile and run a test harness that compares libft functions against libc equivalents. Write your test logic in C.",
  {
...
server.tool(
  "allocation_map",
  "Complete map of which functions allocate heap memory, what they return, and what the caller must free.",
  {},
  async () => {
    const index = getFunctionIndex();
...
server.tool(
  "validate_makefile",
  "Validate the Makefile against all requirements: targets, flags, relinking, bonus separation, library name.",
  {},
  async () => {
...
server.tool(
  "test_gnl",
  "Test get_next_line with custom file content and BUFFER_SIZE. Creates a temp file, compiles with the specified buffer size, and reports each line returned.",
  {
