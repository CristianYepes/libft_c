...
export function parseProject(projectDir: string): FunctionInfo[] {
  ...
  for (const file of cFiles) {
    ...
    for (const [funcName, body] of funcBodies) {
      if (staticFuncs.includes(funcName)) continue;
      if (!funcName.startsWith("ft_") && !funcName.startsWith("get_") && !funcName.startsWith("contains_") && !funcName.startsWith("join_")) continue;
      ...
      let allocationType: string | null = null;
      if (allocates) {
        if (returnType.includes("char **")) allocationType = "array of strings (free each + free array, or use ft_free_split)";
        else if (returnType.includes("char *")) allocationType = "string (free return value)";
        else if (returnType.includes("t_list")) allocationType = "list node (use ft_lstdelone/ft_lstclear)";
        else if (returnType.includes("void *")) allocationType = "generic allocation (free return value)";
        else if (returnType === "void") allocationType = "internal (allocates buffers freed internally)";
      }
...
function extractFunctionBodies(lines: string[]): Map<string, string> {
